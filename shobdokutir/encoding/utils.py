...
def _shift_right(target: List, pos: int, fola_set: set) -> List:
    """
...
def _rearrange_b2u(target: List, prekar_set: Set) -> List:
    ...
    for pr in prekars:
        ...
        target = _shift_right(target, pr, fola_set)
...
def main(): 
    # print(bijoy2unicode("AvuwLZviv ‡cŠuQv‡Z SuywK euvk agx©q chv‡©qi Kgv‡K©i wi‡cÖ‡R‡›UwUf"))
    print(bijoy2unicode("†cÖmwµck‡b wi‡cÖ‡R‡›UwUf P›`«we›`y "))
